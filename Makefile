...
$(LAUNCHER):
	@printf '%s\n' '#!/bin/sh' 'set -e' 'LOCAL_UID="${LOCAL_UID:-$$(id -u)}"' 'LOCAL_GID="${LOCAL_GID:-$$(id -g)}"' 'export LOCAL_UID LOCAL_GID' 'if docker compose version >/dev/null 2>&1; then' '  exec docker compose up -d --build' 'elif command -v docker-compose >/dev/null 2>&1; then' '  exec docker-compose up -d --build' 'else' '  echo "Error: Docker Compose is not available."' '  exit 1' 'fi' > $(LAUNCHER)
	@chmod +x $(LAUNCHER)
