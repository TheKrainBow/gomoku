...
export default function App() {
  ...
  const [applyBoardCapturedBlue, setApplyBoardCapturedBlue] = useState(0)
  const [applyBoardCapturedRed, setApplyBoardCapturedRed] = useState(0)
  const [applyBoardBusy, setApplyBoardBusy] = useState(false)
  const [applyBoardError, setApplyBoardError] = useState('')
  ...
  const effectiveHistoryIndex =
    hoveredHistoryIndex != null
      ...
      : latestHistoryIndex
  const isFrozenToHistory = selectedHistoryIndex >= 0 && selectedHistoryIndex < latestHistoryIndex
  ...
  const activeMoveSuggestion = useMemo(() => {
    if (!status.config.ghost_mode || !moveSuggestion) {
      return null
    }
    if (effectiveHistoryIndex !== latestHistoryIndex) {
      return null
    }
    if (moveSuggestion.next_player !== status.next_player) {
      return null
    }
    if (moveSuggestion.history_len !== history.length) {
      return null
    }
  ...
  const boardRows = useMemo(() => {
    ...
    const showSuggestion = !!activeMoveSuggestion
        ...
        {row.map((cell, colIndex) => (
          (() => {
            ...
            const isSuggestionCell =
              showSuggestion &&
              renderedCell === 0 &&
              colIndex === activeMoveSuggestion.x &&
              rowIndex === activeMoveSuggestion.y
            return (
          <div
  ...
  const handleUndoToHere = async () => {
    if (!isFrozenToHistory || undoBusy || selectedHistoryIndex < 0) {
      return
    }
    setUndoBusy(true)
    try {
      const res = await fetch('/api/undo', {
        ...
        body: JSON.stringify({ history_index: selectedHistoryIndex })
      })
      if (res.ok) {
        const data = await res.json()
        setSelectedHistoryIndex(-1)
  ...
  const handleOpenSnapshot = () => {
    setSnapshotCopyState('')
    setSnapshotText(formatBoardSnapshot(displayedSnapshot))
  ...
  const handleCopySnapshot = async () => {
    try {
      ...
      setSnapshotCopyState('Copy failed')
    }
  }

  const handleOpenApplyBoard = () => {
    setApplyBoardOpen(true)
    setApplyBoardError('')
    setApplyBoardNextPlayer(status.next_player || 1)
    setApplyBoardCapturedBlue(0)
    setApplyBoardCapturedRed(0)
  }

  const handleApplyBoard = async () => {
    let cells
    try {
      cells = parseBoardSnapshotText(applyBoardText)
    } catch (error) {
      setApplyBoardError(error.message || 'Invalid board snapshot')
      return
    }

    setApplyBoardBusy(true)
    setApplyBoardError('')
    try {
      const res = await fetch('/api/board/apply', {
        ...
        body: JSON.stringify({
          cells,
          next_player: Number(applyBoardNextPlayer),
          captured_blue: Number(applyBoardCapturedBlue),
          captured_red: Number(applyBoardCapturedRed)
        })
      })
      const data = await res.json().catch(() => ({}))
      if (!res.ok) {
        setApplyBoardError(data.error || 'Failed to apply board')
        return
      }
      setSelectedHistoryIndex(-1)
      setHoveredHistoryIndex(null)
      setStatus(data)
      setApplyBoardOpen(false)
    } finally {
      setApplyBoardBusy(false)
    }
  }
  ...
  const handleSettingsChange = (field, value) => {
    setStatus((prev) => {
      ...
      if (field === 'ai_depth') {
        nextConfig.ai_min_depth = 1
        nextConfig.ai_max_depth = value
      }
      if (field === 'ai_timeout_ms') {
        nextConfig.ai_time_budget_ms = 0
      }
      const next = {
        ...prev,
        config: nextConfig
    ...
    <div className="page">
      ...
      <div className="layout">
            ...
            <div className="settings-grid">
              ...
              <label>
                ...
                <input
                  type="number"
                  min="0"
                  value={status.config.ai_timeout_ms ?? 0}
        ...
        <section className="board-panel" ref={boardPanelRef}>
          <div className="board-panel-main">
            ...
            <div
              className={`turn-timer suggestion-slot ${activeMoveSuggestion ? 'suggestion-slot-visible' : 'suggestion-slot-hidden'}`}
              aria-hidden={!activeMoveSuggestion}
            >
              {activeMoveSuggestion
                ? `Suggestion: (${activeMoveSuggestion.x}, ${activeMoveSuggestion.y}) best depth ${activeMoveSuggestion.depth}`
                : 'Suggestion: (0, 0) best depth 0'}
            </div>
          </div>
          <div className="board-panel-actions">
            {isFrozenToHistory && (
              <button type="button" onClick={handleUndoToHere} disabled={undoBusy}>
                {undoBusy ? 'Undoing…' : 'Undo to here'}
              </button>
            )}
            <button type="button" onClick={handleOpenApplyBoard}>
              Apply board
            </button>
            <button type="button" onClick={handleOpenSnapshot}>
              Snapshot board
            </button>
          </div>
        </section>
      ...
      {snapshotOpen && (
        <div className="snapshot-modal-backdrop" onClick={() => setSnapshotOpen(false)} role="presentation">
          <div className="snapshot-modal" onClick={(event) => event.stopPropagation()} role="dialog" aria-modal="true">
            <div className="snapshot-modal-header">
              ...
              <button type="button" onClick={() => setSnapshotOpen(false)}>
                Close
              </button>
            </div>
            <textarea readOnly value={snapshotText} className="snapshot-textarea" />
            <div className="snapshot-modal-actions">
              <button type="button" onClick={handleCopySnapshot}>
                Copy
              </button>
              {snapshotCopyState ? <span className="history-empty">{snapshotCopyState}</span> : null}
            </div>
          </div>
        </div>
      )}
      {applyBoardOpen && (
        <div className="snapshot-modal-backdrop" onClick={() => setApplyBoardOpen(false)} role="presentation">
          <div className="snapshot-modal" onClick={(event) => event.stopPropagation()} role="dialog" aria-modal="true">
            <div className="snapshot-modal-header">
              <h2>Apply board</h2>
              <button type="button" onClick={() => setApplyBoardOpen(false)}>
                Close
              </button>
            </div>
            <textarea
              value={applyBoardText}
              onChange={(event) => setApplyBoardText(event.target.value)}
              className="snapshot-textarea"
              placeholder={'{\n\t\t{7, 6, CellBlue},  // 11\n\t}'}
            />
            <div className="apply-board-grid">
              <label>
                Next player
                <select value={applyBoardNextPlayer} onChange={(event) => setApplyBoardNextPlayer(Number(event.target.value))}>
              ...
              <label>
                ...
                <input
                  type="number"
                  min="0"
                  value={applyBoardCapturedBlue}
                  onChange={(event) => setApplyBoardCapturedBlue(Number(event.target.value))}
                />
              </label>
              <label>
                Red captures
                <input
                  type="number"
                  min="0"
                  value={applyBoardCapturedRed}
                  onChange={(event) => setApplyBoardCapturedRed(Number(event.target.value))}
                />
              </label>
            </div>
            <div className="snapshot-modal-actions">
              <button type="button" onClick={handleApplyBoard} disabled={applyBoardBusy}>
                {applyBoardBusy ? 'Applying…' : 'Apply'}
              </button>
              {applyBoardError ? <span className="history-empty">{applyBoardError}</span> : null}
            </div>
          </div>
        </div>
      )}
...
function formatBoardSnapshot(snapshot) {
  if (!snapshot?.board || snapshot.board.length === 0) {
    return '{\n\t}'
  }

  const entries = []
  for (let y = 0; y < snapshot.board.length; y++) {
    for (let x = 0; x < snapshot.board[y].length; x++) {
      ...
      entries.push({
        x,
        y,
        cellLabel: cell === 1 ? 'CellBlue' : 'CellRed',
        moveNumber: snapshot.moveNumbers?.[y]?.[x] || 0
      })
    }
  }
  ...
  const body = entries.map((entry) => {
    const base = `\t\t{${entry.x}, ${entry.y}, ${entry.cellLabel}},`
    return `${base.padEnd(31, ' ')}// ${entry.moveNumber}`
  })
  return `{\n${body.join('\n')}\n\t}`
}

function parseBoardSnapshotText(text) {
  const input = String(text || '')
  const matches = [...input.matchAll(/\{(\d+)\s*,\s*(\d+)\s*,\s*(CellBlue|CellRed)\s*\}\s*,?(?:\s*\/\/\s*(\d+))?/g)]
  if (matches.length === 0) {
    throw new Error('No board cells found')
  }
  ...
  return matches.map((match) => {
    ...
    const cell = match[3]
    const moveNumber = match[4] ? Number(match[4]) : 0
    const key = `${x},${y}`
    if (seen.has(key)) {
      throw new Error(`Duplicate cell: (${x}, ${y})`)
    }
    seen.add(key)
    return { x, y, cell, move_number: moveNumber }
  })
}
