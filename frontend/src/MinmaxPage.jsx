...
const baseStepHelp = [
  ...
  'Step 3: Go up to depth-2 using MAX (red chooses the biggest score).',
  'Step 4: Go up to depth-1 using MIN (blue chooses the smallest score).',
  'Step 5: Go up to root using MAX and pick final move.'
]

const capturedCellsByNode = {
  // B(3,3), W(2,3), B(4,3), W(5,3) => red capture of the two blue stones.
  n7: new Set(['3,3', '4,3'])
}
  ...
  return (
    <div className="page minmax-page">
      ...
      <section className="panel minmax-legend">
        <div>Evaluation perspective: positive = Red is better, negative = Blue is better.</div>
        <div>
          {step <= 4
            ? 'Minmax phase: leaves are evaluated, then values go up by alternating MAX (red) and MIN (blue).'
