...
export default function TrainerPage() {
  ...
  const trainingProgress = useMemo(() => {
    ...
    if (match && match.stage === 'population') {
      const blueIndex = parseContenderIndex(match.blue_id)
      const redIndex = parseContenderIndex(match.red_id)
      if (blueIndex >= 0 && redIndex > blueIndex) {
        let pairBefore = 0
        for (let i = 0; i < blueIndex; i++) {
          pairBefore += populationSize - i - 1
        }
        pairBefore += redIndex - blueIndex - 1
            ...
            <div className="trainer-card">
              ...
              {status.current_match ? (
                <>
                  ...
                  <p>blue={status.current_match.blue_id}</p>
                  <p>red={status.current_match.red_id}</p>
