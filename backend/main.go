...
type applyBoardCellPayload struct {
	X          int    `json:"x"`
	Y          int    `json:"y"`
...
type applyBoardPayload struct {
	Cells        []applyBoardCellPayload `json:"cells"`
	NextPlayer   int                     `json:"next_player"`
	CapturedBlue int                     `json:"captured_blue"`
	CapturedRed  int                     `json:"captured_red"`
}
...
func main() {
	...
	r.Post("/api/undo", func(w http.ResponseWriter, r *http.Request) {
		...
		searchBacklogManager.RequestStop()
		applied, errMsg := controller.UndoToHistoryIndex(payload.HistoryIndex)
		if !applied {
			writeJSON(w, http.StatusBadRequest, map[string]string{"error": errMsg})
			return
		}
	...
	r.Post("/api/board/apply", func(w http.ResponseWriter, r *http.Request) {
		var payload applyBoardPayload
		if err := json.NewDecoder(r.Body).Decode(&payload); err != nil {
			writeJSON(w, http.StatusBadRequest, map[string]string{"error": "invalid payload"})
			return
		}
		searchBacklogManager.RequestStop()
		applied, errMsg := controller.ApplyBoardSnapshot(payload)
		if !applied {
			writeJSON(w, http.StatusBadRequest, map[string]string{"error": errMsg})
			return
		}
...
func controllerSettingsDTO(settings GameSettings) GameSettingsDTO {
	mode := "ai_vs_human"
	if settings.BlueType == PlayerAI && settings.RedType == PlayerAI {
		mode = "ai_vs_ai"
	} else if settings.BlueType == PlayerHuman && settings.RedType == PlayerHuman {
		mode = "human_vs_human"
	} else if settings.BlueType != settings.RedType {
		mode = "ai_vs_human"
	}
	humanPlayer := 0
	if settings.BlueType == PlayerHuman && settings.RedType != PlayerHuman {
		humanPlayer = 1
	} else if settings.RedType == PlayerHuman && settings.BlueType != PlayerHuman {
		humanPlayer = 2
	} else if settings.BlueType == PlayerHuman && settings.RedType == PlayerHuman {
		humanPlayer = 1
	}
