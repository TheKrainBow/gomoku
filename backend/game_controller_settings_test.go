...
func TestUpdateSettingsSwitchToAIVsAIKeepsBoardAndContinuesGame(t *testing.T) {
	...
	updated.BlueType = PlayerAI
	updated.RedType = PlayerAI
	...
	if got := controller.Settings(); got.BlueType != PlayerAI || got.RedType != PlayerAI {
		t.Fatalf("expected settings to switch to ai_vs_ai, got blue=%d red=%d", got.BlueType, got.RedType)
	}
