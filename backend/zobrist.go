...
func UpdateHashAfterMove(state *GameState, move Move, player PlayerColor, captures []Move, prevToMove PlayerColor, prevCapturedBlue, prevCapturedRed int) {
	z := GetZobrist(state.Board.Size())
	hash := state.Hash
	if prevToMove == PlayerRed {
		hash ^= z.side
	...
	hash ^= captureHash(PlayerBlue, prevCapturedBlue)
	hash ^= captureHash(PlayerBlue, state.CapturedBlue)
	hash ^= captureHash(PlayerRed, prevCapturedRed)
	hash ^= captureHash(PlayerRed, state.CapturedRed)
	xorCaptureHashes(&state.HashSym, prevCapturedBlue, state.CapturedBlue, prevCapturedRed, state.CapturedRed)
...
func computeSymmetricHashes(state GameState) (uint64, [8]uint64) {
	...
	hash ^= captureHash(PlayerRed, state.CapturedRed)
	for i := range sym {
		sym[i] ^= captureHash(PlayerBlue, state.CapturedBlue)
		sym[i] ^= captureHash(PlayerRed, state.CapturedRed)
	}
	return hash, sym
}
...
func xorCaptureHashes(sym *[8]uint64, prevBlue, newBlue, prevRed, newRed int) {
	for i := range sym {
		sym[i] ^= captureHash(PlayerBlue, prevBlue)
		sym[i] ^= captureHash(PlayerBlue, newBlue)
		sym[i] ^= captureHash(PlayerRed, prevRed)
		sym[i] ^= captureHash(PlayerRed, newRed)
	}
}
