...
func (tt *TranspositionTable) Clear() {
	...
	if tt.debugBoards != nil {
		for i := range tt.debugBoards {
			tt.debugBoards[i] = Board{}
		}
	}
	if tt.debugLines != nil {
		for i := range tt.debugLines {
			tt.debugLines[i] = nil
		}
	}
	tt.gen.Store(1)
...
func (tt *TranspositionTable) ProbeDebugBoard(key uint64, heuristicHash uint64) (Board, bool) {
	if tt == nil {
		return Board{}, false
	}
	stripe := tt.stripeIndexForKey(key)
	...
	for i := 0; i < tt.buckets; i++ {
		...
		board := tt.debugBoards[idx]
		if board.Size() <= 0 {
			return Board{}, false
		}
		return board.Clone(), true
	}
	return Board{}, false
}

func (tt *TranspositionTable) ProbeDebugLine(key uint64, heuristicHash uint64) (*SearchDebugLine, bool) {
	if tt == nil {
		return nil, false
	}
	stripe := tt.stripeIndexForKey(key)
	...
	for i := 0; i < tt.buckets; i++ {
		...
		line := tt.debugLines[idx]
		if line == nil {
			return nil, false
		}
		return cloneSearchDebugLine(line), true
	}
	return nil, false
}

func (tt *TranspositionTable) storeDebugBoard(idx int, board *Board) {
	if tt == nil || idx < 0 || idx >= len(tt.entries) {
		return
	}
	if board == nil || board.Size() <= 0 {
		if tt.debugBoards != nil {
			tt.debugBoards[idx] = Board{}
		}
		return
	}
	if tt.debugBoards == nil {
		tt.debugBoards = make([]Board, len(tt.entries))
	}
	tt.debugBoards[idx] = board.Clone()
}

func (tt *TranspositionTable) storeDebugLine(idx int, line *SearchDebugLine) {
	if tt == nil || idx < 0 || idx >= len(tt.entries) {
		return
	}
	if line == nil {
		if tt.debugLines != nil {
			tt.debugLines[idx] = nil
		}
		return
	}
	if tt.debugLines == nil {
		tt.debugLines = make([]*SearchDebugLine, len(tt.entries))
	}
	tt.debugLines[idx] = cloneSearchDebugLine(line)
}
...
func (tt *TranspositionTable) Store(key uint64, heuristicHash uint64, depth int, value float64, flag TTFlag, best Move, meta TTMeta) (replaced bool, overwrote bool) {
	...
	for i := 0; i < tt.buckets; i++ {
		...
		tt.storeDebugBoard(idx, meta.DebugBoard)
		tt.storeDebugLine(idx, meta.DebugLine)
		return false, true
	}
	...
	tt.storeDebugBoard(victim, meta.DebugBoard)
	tt.storeDebugLine(victim, meta.DebugLine)
	return true, false
}
