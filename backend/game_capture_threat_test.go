...
func TestGameStopsBeforeTenthCaptureWhenThreatExists(t *testing.T) {
	...
	// Red already has 8 captures. If Red plays at (3,4), it captures
	// the Blue pair (4,4)-(5,4) thanks to the Red stone at (6,4).
	g.state.CapturedRed = 8
	g.state.Board.Set(4, 4, CellBlue)
	g.state.Board.Set(5, 4, CellBlue)
	g.state.Board.Set(6, 4, CellRed)
	...
	if g.state.Status != StatusRedWon {
		t.Fatalf("expected Red to win via forced capture move, got status=%v", g.state.Status)
	}
	...
	if g.state.Board.At(3, 4) != CellRed {
	...
	if last.Player != PlayerRed || !last.Move.Equals(Move{X: 3, Y: 4}) {
		t.Fatalf("expected last history move to be forced Red capture at (3,4), got player=%v move=%+v", last.Player, last.Move)
	}
