...
func (gc *GameController) UndoToHistoryIndex(index int) (bool, string) {
	gc.mu.Lock()
	defer gc.mu.Unlock()
	return gc.game.UndoToHistoryIndex(index)
}

func (gc *GameController) ApplyBoardSnapshot(payload applyBoardPayload) (bool, string) {
	gc.mu.Lock()
	defer gc.mu.Unlock()
	return gc.game.ApplyBoardSnapshot(payload)
}
