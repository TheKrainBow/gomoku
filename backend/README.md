...
- Candidates are ordered by a fast heuristic (`orderCandidates`) and capped dynamically by depth and position type.
...
- Search alternates between players, with **Blue always maximizing** and **Red minimizing**.
- The terminal evaluation is `evaluateStateHeuristic`:
  - Returns a large positive score for Blue win and a large negative score for Red win.
  - Otherwise, returns a board heuristic in the same Blue-positive / Red-negative convention.
...
- `ForbidDoubleThreeBlue`, `ForbidDoubleThreeRed`: legal move restrictions.
- `BlueHeuristics`, `RedHeuristics` (optional): per-AI heuristic overrides. If omitted, AI uses `Config.Heuristics` from `backend/config.go`.
...
- `POST /api/start` accepts optional per-player overrides under:
  - `settings.blue_heuristics`
  - `settings.red_heuristics`

When these fields are not provided, both AIs use backend defaults.

## Arbiter Middleware API

The backend also exposes a lightweight protocol compatible with `gomoku-middleware-arbiter`:

- `GET /arbiter/ping`
- `GET /arbiter/start?sessionid=...&you_are=black|white[&force=1][&board_size=19][&win_length=5]...`
- `GET /arbiter/play?sessionid=...&gstatus=playing|black_win|white_win|draw[&move=181][&board=0,0,...]`
- `GET /arbiter/stop`

Notes:

- `sessionid` and `session_id` are both accepted.
- `board` is a flat `board_size * board_size` array of cell ids:
  - `0 = empty`
  - `1 = black`
  - `2 = white`
- `play` returns either:
  - `{ as_played: true, move, board, turn, gstatus }`
  - `{ as_played: false, because, msglog? }`
